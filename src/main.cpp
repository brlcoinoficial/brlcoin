...
#if defined(NDEBUG)
# error "Brlcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcb2ae9d14407fd7bfb1c27feeb5868173748d6d0118b8f30056767ea4dc86da6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Brlcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Brlcoin Signed Message:\n";
...
{
    // Brlcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Brlcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Brlcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1527617728, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "inicio da moeda brlcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f6b0255acb03a23023a19bb797318430642093e4afad6032a1bb9f5d9b92e524f1ce3014afabf4e078421339f16732bbdc0a849a99d8747227e1b6511c7b9cbf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527617728;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1000244;
        ...
        assert(block.hashMerkleRoot == uint256("0x08e138c9a770a3424cf1b285ef6392992800b289c7fb133e84cf7ff29e41892c"));
...
unsigned char pchMessageStart[4] = { 0x99, 0x12, 0x04, 0xc3 }; // Brlcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BrlcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BrlcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BrlcoinMiner(CWallet *pwallet)
{
    printf("BrlcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("brlcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BrlcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BrlcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BrlcoinMiner, pwallet));
}
